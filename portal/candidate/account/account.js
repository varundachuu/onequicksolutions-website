(() => {
  ...
  const toggleProfileButton = document.querySelector("#account-toggle-profile");
  ...
  async function init() {
    ...
    profileVisible = account.profileStatus === "active";
    toggleProfileButton.textContent = profileVisible ? "Deactivate Profile" : "Activate Profile";
    toggleProfileButton.disabled = account.profileStatus === "not_created";
  ...
  toggleProfileButton.addEventListener("click", async () => { if (!currentSession) return; const nextVisibility = !profileVisible; if (!window.confirm(nextVisibility ? "Activate this profile for hiring visibility?" : "Deactivate this profile so it is hidden from hiring?")) return; toggleProfileButton.disabled = true; try { const response = await fetch("/api/candidate/profile/status", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email: currentSession.email, isVisibleForHiring: nextVisibility }) }); const result = await response.json().catch(() => ({})); if (!response.ok) throw new Error(result.message || "Unable to update profile visibility."); profileVisible = nextVisibility; toggleProfileButton.textContent = profileVisible ? "Deactivate Profile" : "Activate Profile"; setActionStatus(result.message, "success"); } catch (error) { setActionStatus(error.message, "error"); } finally { toggleProfileButton.disabled = false; } });
